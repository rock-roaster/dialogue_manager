...
func _dialogue_process() -> void:
	...
	var push_point_04: Control = get_data("node_04") as Control
	...
	add_callable(character_02.change_body_alpha.bind(0.0))

	add_text("八百标兵奔北坡，北坡炮兵并排跑。\n炮兵怕把标兵碰，标兵怕碰炮兵炮。")\
		.set_popup_parent(push_point_04).set_ms_per_char(0.0)
